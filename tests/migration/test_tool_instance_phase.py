...
- The source list of "things to migrate" comes from the workflow remap
...
def test_happy_path_creates_instance_then_patches_metadata():
    ...
    src.instances[SRC_WF] = [
        _src_ti(
            ...
            {"llm": "My OpenAI", "embedding": "MyEmb", "tenant_id": "src-org"},
        )
    ]
    tgt = FakeClient()
    ctx = _ctx(src, tgt, remap=_seed_remap())

    result = ToolInstancePhase(ctx).run(MigrationReport())
    ...
    # PATCH carries the source metadata verbatim (backend handles name→UUID).
