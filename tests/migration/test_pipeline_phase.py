...
- DEFAULT and APP types are skipped (out of migration scope).
...
def test_happy_path_creates_pipeline_with_remapped_workflow():
    ...
    result = PipelinePhase(ctx).run(MigrationReport())
...
def test_abort_on_name_conflict_raises():
    ...
    with pytest.raises(NameConflictError):
        PipelinePhase(ctx).run(MigrationReport())
...
def test_extra_source_keys_log_warning_not_failure(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="unstract.migration.phases.pipeline"):
        result = PipelinePhase(ctx).run(MigrationReport())
